...
def test_integration_liquidity_receiver_free_marginal_v1__updates_pool_info(
    ...
    # free should zero out timestamp and shares so cant be reused
    info.shares = 0
    info.blockTimestamp = 0

    # free liquidity to alice
    liquidity_receiver.freeMarginalV1(alice.address, sender=finalizer)

    info_after = liquidity_receiver.marginalV1PoolInfo()
...
@pytest.mark.parametrize("init_with_sqrt_price_lower_x96", [True, False])
@pytest.mark.parametrize("percent_thru_range", [1.0])
def test_integration_liquidity_receiver_free_marginal_v1__transfers_funds(
    margv1_liquidity_receiver_and_pool_finalized,
    factory,
    sender,
    alice,
    ...
    pool_token = project.Token.at(info.poolAddress)
    pool_token.balanceOf(alice.address) == 0
    pool_token.balanceOf(liquidity_receiver.address) == info.shares
    ...
    # check LP token shares transferred
    pool_token.balanceOf(liquidity_receiver.address) == 0
    pool_token.balanceOf(alice.address) == info.shares
