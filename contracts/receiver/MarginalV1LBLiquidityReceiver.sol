    ...
    Multicall
{
    ...
    function checkParams(ReceiverParams memory params) public {
        if (params.treasuryRatio > 1e6 || params.uniswapRatio > 1e6) revert InvalidRatio();
    ...
    function fullTickRange(uint24 fee) private view returns (int24 tickLower, int24 tickUpper) {
    ...
    function maximumLeverage(uint24 maintenance) private view returns (uint256) {
    ...
    function checkDeadline(uint96 blockTimestamp, uint96 duration) internal view {
        uint96 delta;
        unchecked { delta = _blockTimestamp() - blockTimestamp; }
    ...
    function notifyRewardAmounts(uint256 amount0, uint256 amount1) external {
        ...
        (uint160 sqrtPriceX96,,,,,,, bool finalized) = IMarginalV1LBPool(pool).state();
        ...
        if (amount0Treasury > 0) pay(poolKey.token0, address(this), params.treasuryAddress, amount0Treasury);
        if (amount1Treasury > 0) pay(poolKey.token1, address(this), params.treasuryAddress, amount1Treasury);
    }
    ...
    function getAmountsDesired(uint160 sqrtPriceX96, uint256 amount0, uint256 amount1) public pure returns (uint256 amount0Desired, uint256 amount1Desired) {
        ...
        (amount0Desired, amount1Desired) = LiquidityMath.toAmounts(liquidity, sqrtPriceX96);
    }

    /// @inheritdoc IMarginalV1LBLiquidityRecipient
    function mintUniswapV3() external payable returns (address uniswapV3Pool, uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1) {
        ReceiverParams memory params = receiverParams;

        (uint256 _reserve0, uint256 _reserve1) = (reserve0, reserve1);
        if (_reserve0 == 0 && _reserve1 == 0) revert InvalidReserves();

        if (uniswapV3PoolInfo.poolAddress != address(0)) revert LiquidityAdded();

        // get Uniswap v3 NFT position manager from deployer
        address uniswapV3NonfungiblePositionManager = IMarginalV1LBLiquidityReceiverDeployer(deployer).uniswapV3NonfungiblePositionManager();

        // create uniswap v3 pool if necessary
        (uint160 sqrtPriceX96,,,,,,, bool finalized) = IMarginalV1LBPool(pool).state();
        uniswapV3Pool = IUniswapV3NonfungiblePositionManager(uniswapV3NonfungiblePositionManager).createAndInitializePoolIfNecessary(
            token0,
            token1,
            params.uniswapV3Fee,
            sqrtPriceX96
        ...
        (uint256 amount0Desired, uint256 amount1Desired) = getAmountsDesired(sqrtPriceX96, amount0UniswapV3, amount1UniswapV3);
        if (amount0Desired > amount0UniswapV3) pay(token0, msg.sender, address(this), amount0Desired - amount0UniswapV3);
        if (amount1Desired > amount1UniswapV3) pay(token1, msg.sender, address(this), amount1Desired - amount1UniswapV3);
        ...
        IERC20(token0).safeIncreaseAllowance(uniswapV3NonfungiblePositionManager, amount0Desired);
        IERC20(token1).safeIncreaseAllowance(uniswapV3NonfungiblePositionManager, amount1Desired);

        (tokenId, liquidity, amount0, amount1) = IUniswapV3NonfungiblePositionManager(uniswapV3NonfungiblePositionManager).mint(
    ...
    /// @inheritdoc IMarginalV1LBLiquidityRecipient
    function mintMarginalV1() external payable returns (address marginalV1Pool, uint256 shares, uint256 amount0, uint256 amount1) {
        ...
        if (marginalV1PoolInfo.poolAddress != address(0)) revert LiquidityAdded();

        // set reserves to zero
        reserve0 = 0;
        reserve1 = 0;

        address marginalV1Factory = IMarginalV1LBLiquidityReceiverDeployer(deployer).marginalV1Factory();
        marginalV1Pool = IMarginalV1Factory(marginalV1Factory).getPool(token0, token1, params.marginalV1Maintenance, uniswapV3Pool);
        ...
        (uint256 amount0Desired, uint256 amount1Desired) = getAmountsDesired(sqrtPriceX96, _reserve0, _reserve1);
        if (amount0Desired > _reserve0) pay(token0, msg.sender, address(this), amount0Desired - _reserve0);
        if (amount1Desired > _reserve1) pay(token1, msg.sender, address(this), amount1Desired - _reserve1);

        if (initialize) {
            address marginalV1PoolInitializer = IMarginalV1LBLiquidityReceiverDeployer(deployer).marginalV1PoolInitializer();

            // use initializer to create pool and add liquidity
            IERC20(token0).safeIncreaseAllowance(marginalV1PoolInitializer, amount0Desired);
            IERC20(token1).safeIncreaseAllowance(marginalV1PoolInitializer, amount1Desired);

            // calculate (roughly) amounts{0,1} desired accounting for liquidity burn plus swap
            uint128 liquidityDesired = LiquidityAmounts.getLiquidityForAmounts(sqrtPriceX96, amount0Desired, amount1Desired);
            ...
            (amount0Desired, amount1Desired) = LiquidityMath.toAmounts(liquidityDesired, sqrtPriceX96);

            int256 _amount0;
            int256 _amount1;
            (marginalV1Pool, shares, _amount0, _amount1) = IMarginalV1PoolInitializer(marginalV1PoolInitializer).createAndInitializePoolIfNecessary(
            ...
            address marginalV1Router = IMarginalV1LBLiquidityReceiverDeployer(deployer).marginalV1Router();

            // use router to add liquidity
            IERC20(token0).safeIncreaseAllowance(marginalV1Router, amount0Desired);
            IERC20(token1).safeIncreaseAllowance(marginalV1Router, amount1Desired);

            (shares, amount0, amount1) = IMarginalV1Router(marginalV1Router).addLiquidity(
    ...
    function freeUniswapV3(address recipient) external {
        ...
        IUniswapV3NonfungiblePositionManager(uniswapV3NonfungiblePositionManager).transferFrom(
            address(this), recipient, info.tokenId
        );
    }
